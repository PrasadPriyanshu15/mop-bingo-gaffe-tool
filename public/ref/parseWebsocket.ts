...
// normalized rows for a chosen denomination.
//
// Shape of the payload (see plan):
...
export interface PaytableInfoRoot {
  PaytableInfo?: {
    bingoPatterns?: {
      ...
      denoms?: Record<
        string,
        { lines?: Record<string, { multipliers?: Record<string, { patterns?: SliceRow[] }> }> }
      >;
    };
  };
}
...
export function extractPayload(pasted: string): PaytableInfoRoot {
  ...
  if (
    ...
    typeof value === "object" &&
    "data" in value &&
    (value as { data?: { responseText?: unknown } }).data?.responseText !== undefined
  ) {
 ...
 * Each slice row is resolved against the master list by uuid; info.Credits overrides
 * the master Credits when present.
 */
export function normalizeWebsocket(root: PaytableInfoRoot, denom: string): MultiplierMap {
